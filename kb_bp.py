...
def kb_index():
    ...
    return render_template(
        "kb_list.html", businesses=businesses, business_id=business_id, q=q, entries=entries
    )
...
def kb_new():
    ...
    businesses = list_businesses(limit=500)
    return render_template(
        "kb_edit.html", mode="new", businesses=businesses, entry=None, business_id=business_id
    )
...
def kb_suggestions():
    ...
    from core.kb_suggestions import is_configured, suggest_kb_entries
...
def kb_import(business_id: int):
    ...
    try:
        ...
        if imported:
            flash(
                f"Imported {imported} KB entr{'y' if imported == 1 else 'ies'}"
                + (f" ({errors} skipped due to missing fields)" if errors else "")
                + ".",
                "ok",
            )
        else:
            flash(
                "No entries imported. Check your CSV format — columns must be: question, answer, tags.",
                "err",
            )

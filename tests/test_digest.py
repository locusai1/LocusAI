...
def _seed_activity(test_db, business_id):
    from core.db import create_appointment, create_session, get_conn

    with get_conn() as con:
        # sessions across channels
        for ch in ("voice", "voice", "web", "sms"):
            con.execute(
                "INSERT INTO sessions (business_id, channel) VALUES (?, ?)", (business_id, ch)
            )
        # voice calls with intents + containment
        con.execute(
            "INSERT INTO voice_calls (business_id, retell_call_id, direction, call_intent, containment) "
            "VALUES (?, 'c1', 'inbound', 'booking', 1)",
            (business_id,),
        )
        con.execute(
            "INSERT INTO voice_calls (business_id, retell_call_id, direction, call_intent, containment) "
            "VALUES (?, 'c2', 'inbound', 'booking', 1)",
            (business_id,),
        )
        con.execute(
            "INSERT INTO voice_calls (business_id, retell_call_id, direction, call_intent, containment) "
            "VALUES (?, 'c3', 'inbound', 'hours', 0)",
            (business_id,),
        )
        con.execute(
            "INSERT INTO escalations (business_id, reason, status) VALUES (?, 'angry', 'pending')",
            (business_id,),
        )
        con.commit()
    create_appointment(

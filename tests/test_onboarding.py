...
def _empty_business(test_db, name="Empty Co"):
    ...
    with get_conn() as con:
        cur = con.cursor()
        cur.execute("INSERT INTO businesses (name, slug, tenant_key) VALUES (?, ?, ?)",
        ...
        with patch("core.db.DB_PATH", test_db):
            ...
            with get_conn() as con:
                con.execute("INSERT INTO services (business_id, name, duration_min, price, active) "
                            "VALUES (?, 'Haircut', 30, 25, 1)", (bid,))
                con.commit()
            create_appointment(business_id=bid, customer_name="A", phone="+14150000000",

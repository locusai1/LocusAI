...
def voice_settings(test_db, sample_business):
    ...
    with patch("core.db.DB_PATH", test_db):
        ...
        with get_conn() as conn:
            ...
            row = conn.execute(
                "SELECT * FROM voice_settings WHERE business_id = ?", (sample_business["id"],)
            ).fetchone()
...
class TestSignatureVerification:
    ...
    def test_invalid_signature_rejected(self, mock_retell_webhook_secret):
        ...
        assert (
            verify_retell_signature(payload, invalid_signature, mock_retell_webhook_secret) is False
        )
...
class TestVoiceCallRecords:
    ...
    def test_update_voice_call(self, test_db, sample_business):
        ...
        with patch("core.db.DB_PATH", test_db):
            from core.db import transaction
            from core.voice import create_voice_call_record, get_voice_call, update_voice_call
...
class TestWebhookHandlers:
    ...
    def test_handle_call_started(self, test_db, sample_business, voice_webhook_payload):
        ...
        with patch("core.db.DB_PATH", test_db):
            ...
            from core.voice import get_voice_call, handle_call_started
    ...
    def test_handle_call_ended(
        self, test_db, sample_business, voice_webhook_payload, voice_call_ended_payload
    ):
        """call_ended event updates voice_calls record."""
        with patch("core.db.DB_PATH", test_db):
            voice_webhook_payload["call"]["metadata"]["business_id"] = sample_business["id"]

            from core.voice import get_voice_call, handle_call_ended, handle_call_started
...
class TestVoiceAIPrompt:
    ...
    def test_voice_prompt_is_shorter(self):
        ...
        assert (
            "brevity" in prompt.lower()
            or "short" in prompt.lower()
            or "1-2 sentences" in prompt.lower()
        )
...
class TestVoiceIntegration:
    ...
    def test_full_inbound_call_flow(
        self, test_db, sample_business, voice_webhook_payload, voice_call_ended_payload
    ):
...
class TestCallerRecognition:
    ...
    def test_voice_prompt_with_customer_info(self):
        ...
        assert (
            "returning" in prompt.lower()
            or "previous" in prompt.lower()
            or "visit" in prompt.lower()
        )
...
class TestExpandedIntents:
    ...
    def test_format_appointments_for_voice(self):
        ...
        appointments = [{"service": "Haircut", "start_at": future_date, "status": "confirmed"}]
    ...
    def test_cancel_caller_appointment(self, test_db, sample_business):
        ...
        with patch("core.db.DB_PATH", test_db):
            ...
            from core.db import get_appointment_by_id, get_conn

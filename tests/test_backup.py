...
class TestSnapshot:
    def test_snapshot_is_valid_copy(self, test_db, sample_business, tmp_path):
        ...
        try:
            n = con.execute("SELECT COUNT(*) FROM businesses WHERE id=?",
                            (sample_business["id"],)).fetchone()[0]
        finally:
            con.close()
        assert n == 1
...
class TestRotation:
    def test_keeps_newest_n(self, tmp_path):
        ...
        remaining = sorted(os.path.basename(f) for f in
                           __import__("glob").glob(os.path.join(d, "receptionist-*.db")))
        assert deleted == 3

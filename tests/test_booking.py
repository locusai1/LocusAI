...
def test_booking_commit_basic():
    ...
    with get_conn() as con:
        con.execute(
            "INSERT OR IGNORE INTO businesses(name,slug) VALUES('Test Clinic','test_clinic')"
        )
        ...
        con.execute(
            "INSERT OR IGNORE INTO services(business_id,name,duration_min,price,active) VALUES(?,?,?,?,1)",
            (bid, "Checkup", 30, "£75"),
        )
        ...
        for w in range(0, 7):
            if w <= 4:  # Mon-Fri open
                con.execute(
                    """INSERT OR IGNORE INTO business_hours(business_id,weekday,open_time,close_time,closed)
                               VALUES(?,?,?,?,0)""",
                    (bid, w, "09:00", "18:00"),
                )
            elif w == 5:  # Sat 10-14
    ...
    with get_conn() as con:
        row = con.execute(
            """SELECT customer_name, phone, service, start_at, status, external_provider_key

...
def _seed_questions(test_db, business_id, texts):
    ...
    with get_conn() as con:
        ...
        for t in texts:
            cur.execute(
                "INSERT INTO messages (session_id, sender, text) VALUES (?, 'user', ?)", (sid, t)
            )
        con.commit()
...
class TestParse:
    ...
    def test_drops_incomplete_items(self):
        ...
        out = _parse(
            '{"suggestions":[{"question":"Q","answer":""},{"question":"Q2","answer":"A2"}]}'
        )
        ...
        with patch.object(kbs, "OPENAI_API_KEY", "sk-test"), patch("core.db.DB_PATH", test_db):
            ...
            with get_conn() as con:
                con.execute(
                    ...
                    "VALUES (?, 'Are you open Sundays?', 'No, closed Sundays.', 1)",
                    (bid,),
                )
                con.commit()
            llm_json = (
                '{"suggestions":[{"question":"Do you do balayage?","answer":"Yes we do."},'
...
class TestEndpoints:
    ...
    def test_add_suggestion_creates_entry(self, authenticated_client, sample_business, test_db):
        ...
        with patch("core.db.DB_PATH", test_db):
            ...
            with get_conn() as con:
                row = con.execute(
                    "SELECT * FROM kb_entries WHERE business_id=? AND question=?",
                    (sample_business["id"], "Do you offer gift cards?"),
                ).fetchone()

...
def _admin_count(test_db, email):
    ...
    with get_conn() as con:
        return con.execute(
            "SELECT COUNT(*) FROM users WHERE email=? COLLATE NOCASE AND role='admin'", (email,)
        ).fetchone()[0]
...
class TestEnsureAdmin:
    def test_creates_when_missing(self, test_db):
        ...
        with (
            ...
            patch.dict(
                "os.environ", {"ADMIN_EMAIL": "owner@x.com", "ADMIN_PASSWORD": "StrongPass1"}
            ),
        ):
            ...
            with get_conn() as con:
                h = con.execute(
                    "SELECT password_hash FROM users WHERE email='owner@x.com'"
                ).fetchone()[0]
    ...
    def test_noop_when_env_unset(self, test_db):
        ...
        with patch("core.db.DB_PATH", test_db), patch.dict(os.environ, {}, clear=False):

...
def _calculate_frustration_score(
    pattern_matches: int, word_analysis: Dict, punct_analysis: Dict, message_count: int = 1
) -> float:
...
def analyze_sentiment(
    text: str, conversation_history: Optional[List[Dict]] = None, failed_attempts: int = 0
) -> SentimentResult:
    """
    ...
    if human_matches > 0:
        return SentimentResult(
            ...
            details={"matched_patterns": human_patterns, "trigger": "human_request"},
        )
    ...
    elif positive_matches > 0 and any(
        word in text_lower for word in ["thank", "thanks", "bye", "goodbye"]
    ):
        intent = IntentType.GRATITUDE if "thank" in text_lower else IntentType.FAREWELL
    elif any(
        word in text_lower for word in ["hi", "hello", "hey", "good morning", "good afternoon"]
    ):
    ...
    elif urgency_matches >= 2 and any(
        word in text_lower for word in ["emergency", "urgent", "pain", "bleeding"]
    ):
        triggers_escalation = True
...
def summarize_conversation(messages: List[Dict]) -> str:
    ...
    if first_msg:
        summary_parts.append(
            f"Initial inquiry: {first_msg[:100]}{'...' if len(first_msg) > 100 else ''}"
        )
    ...
    if len(user_messages) > 1:
        ...
        if last_msg:
            summary_parts.append(
                f"Latest message: {last_msg[:100]}{'...' if len(last_msg) > 100 else ''}"
            )

...
def build_digest(business_id: int, days: int = 7) -> Dict[str, Any]:
    ...
    with get_conn() as con:
        ...
        out["calls"] = n("SELECT COUNT(*) FROM sessions WHERE business_id=? AND channel='voice' AND created_at>=?", (business_id, since))
        out["chats"] = n("SELECT COUNT(*) FROM sessions WHERE business_id=? AND channel='web' AND created_at>=?", (business_id, since))
        out["sms"] = n("SELECT COUNT(*) FROM sessions WHERE business_id=? AND channel='sms' AND created_at>=?", (business_id, since))
        out["bookings"] = n("SELECT COUNT(*) FROM appointments WHERE business_id=? AND created_at>=?", (business_id, since))
        out["cancellations"] = n("SELECT COUNT(*) FROM appointments WHERE business_id=? AND status='cancelled' AND created_at>=?", (business_id, since))
        out["escalations"] = n("SELECT COUNT(*) FROM escalations WHERE business_id=? AND created_at>=?", (business_id, since))
        ...
        total_voice = n("SELECT COUNT(*) FROM voice_calls WHERE business_id=? AND created_at>=?", (business_id, since))
        contained = n("SELECT COUNT(*) FROM voice_calls WHERE business_id=? AND created_at>=? AND containment=1", (business_id, since))
...
def send_digest(business_id: int, *, force: bool = False) -> bool:
    ...
    with get_conn() as con:
        biz = con.execute("SELECT * FROM businesses WHERE id=? AND archived=0", (business_id,)).fetchone()
    if not biz:
        return False
    biz = dict(biz)
...
def send_weekly_digests() -> int:
    ...
    with get_conn() as con:
        biz_ids = [r["id"] for r in con.execute(
            "SELECT id FROM businesses WHERE archived=0").fetchall()]
    for bid in biz_ids:
        with get_conn() as con:
            already = con.execute(
                "SELECT 1 FROM digest_log WHERE business_id=? AND period_start=?",
                (bid, period)).fetchone()
        if already:
            continue
        if send_digest(bid):

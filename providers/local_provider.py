...
class LocalProvider(Provider):
    ...
    def _is_closed(self, con, day: datetime) -> bool:
        ...
        return bool(
            con.execute(
                "SELECT 1 FROM closures WHERE business_id=? AND date=?", (self.business_id, d)
            ).fetchone()
        )
    ...
    def fetch_slots(self, service_id: Optional[int], date_str: str) -> List[str]:
        ...
        with get_conn() as con:
            ...
            if self._is_closed(con, day):
                return []
            hours = self._day_hours(con, day)
            if not hours:
                return []
            open_h, close_h = hours
            h1, m1 = map(int, open_h.split(":"))
            h2, m2 = map(int, close_h.split(":"))
            ...
            for a in appts:
                try:
                    ...
                    try:
                        a_start = datetime.fromisoformat(a["start_at"])
                    except:
                        continue
                adur = dur
                if a["service"]:
                    row = con.execute(
                        ...
                        (self.business_id, a["service"]),
                    ).fetchone()
                    if row:
                        adur = row["duration_min"]

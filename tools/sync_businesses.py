...
def main():
    ...
    for b in rows:
        ...
        meta = {
            ...
            "logo_path": (
                cur.execute("SELECT logo_path FROM businesses WHERE id=?", (bid,)).fetchone()[
                    "logo_path"
                ]
            ),
        ...
        # export services.csv
        svc = cur.execute(
            "SELECT name,duration_min,price,active,external_id FROM services WHERE business_id=? ORDER BY name",
            (bid,),
        ).fetchall()
        with open(os.path.join(data_dir, "services.csv"), "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["name", "duration_min", "price", "active", "external_id"])
            for r in svc:
                w.writerow(
                    [
                        r["name"],
        ...
        integ = cur.execute(
            "SELECT provider_key,status,account_json FROM integrations WHERE business_id=?", (bid,)
        ).fetchall()
        export = [
            {
                "provider_key": i["provider_key"],
                "status": i["status"],
                "account": (json.loads(i["account_json"]) if i["account_json"] else {}),
            }
            for i in integ
        ]
        ...
        cur.execute(
            "UPDATE businesses SET files_path=?, static_path=? WHERE id=?",
            (f"businesses/{slug}", f"tenants/{slug}", bid),
        )
    con.commit()
